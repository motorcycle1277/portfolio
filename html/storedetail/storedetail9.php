...
try{
foreach($pdo->query('select * from information where storeID = 9') as $row){
...
try{
  foreach ($pdo->query('select * from review where storeID = "9"') as $row2) {
    // code...
...
<form action="../reviewpost.php" method="post">
  ...
  <input type="hidden" name="storeid" value="9">
